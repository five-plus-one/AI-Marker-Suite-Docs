---
title: 支持平台 - AI 批改助手
description: AI 批改助手支持的在线阅卷平台，包括智学网、七天网络、好分数和五岳阅卷。
---
...
| | 智学网 | 七天网络 | 好分数 | 五岳阅卷 | 华翰云 |
|--|--------|---------|-------|---------|-------|
| 网址 | zhixue.com | 7net.cc / qt7.net | haofenshu.com | wylkyj.com | yunyuejuan.net |
| 适配状态 | ✅ 完整支持 | ✅ 完整支持（含新旧 UI） | ✅ 完整支持 | ✅ 完整支持 | ✅ 完整支持 |
| 答题卡渲染 | `<img>` 标签 | `<img>` / Canvas | SVG `<image>` | `<img>` 标签 | `<img>` 标签 |
| 批改流程 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 |
