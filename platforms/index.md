---
title: 支持平台 - AI 批改助手
description: AI 批改助手支持的在线阅卷平台，包括智学网、七天网络、好分数、五岳阅卷、华翰云、光大阅卷、云阅卷和新教育。
---
...
| | 智学网 | 七天网络 | 好分数 | 五岳阅卷 | 华翰云 | 光大阅卷 | 云阅卷 | 新教育 |
|--|--------|---------|-------|---------|-------|---------|-------|-------|
| 网址 | zhixue.com | 7net.cc / qt7.net | haofenshu.com | wylkyj.com | yunyuejuan.net | pj.yixx.cn | 内网部署 | xinjiaoyu.com |
| 适配状态 | ✅ 完整支持 | ✅ 完整支持（含新旧 UI） | ✅ 完整支持 | ✅ 完整支持 | ✅ 完整支持 | ✅ 完整支持 | ✅ 完整支持 | ✅ 完整支持（考试+作业） |
| 答题卡渲染 | `<img>` 标签 | `<img>` / Canvas | SVG `<image>` | `<img>` 标签 | `<img>` 标签 | Canvas | `<img>` 标签 | Canvas / API |
| 分数输入 | 输入框 | 输入框 | 输入框 | 输入框 | 输入框 | 点击选择 | 输入框 | 输入框 |
| 批改流程 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 点击 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 |
