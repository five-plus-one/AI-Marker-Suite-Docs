...
| | 智学网 | 七天网络 | 好分数 | 五岳阅卷 |
|--|--------|---------|-------|---------|
| 网址 | zhixue.com | 7net.cc / qt7.net | haofenshu.com | wylkyj.com |
| 适配状态 | ✅ 完整支持 | ✅ 完整支持（含新旧 UI） | ✅ 完整支持 | ✅ 完整支持 |
| 答题卡渲染 | `<img>` 标签 | `<img>` / Canvas | SVG `<image>` | `<img>` 标签 |
| 批改流程 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 | 识别 → 打分 → 填入 → 提交 |
